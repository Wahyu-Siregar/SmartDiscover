...
export function ResultsSection({ result, values = { text: "", targetCount: 15, agenticMode: "auto" }, sourceText = "", spotifyStatus, spotifyConnected = false, onSpotifyDisconnected = () => {}, onRefined = () => {} }: ResultsSectionProps) {
  ...
  return <section className="result-section mx-auto w-full max-w-[800px] px-4 pb-12" aria-labelledby="results-title">
    ...
    <div>
      <p className="result-kicker">{t("resultsKicker")}</p>
      <h2 id="results-title" className="font-display">{t("resultsTitle")}</h2>
      {count > 0 && <p className="result-lead">{t("resultLeadFound", { count: String(count) })}</p>}
    </div>
    {!count ? <p className="mt-3 text-muted-foreground">{t("emptyNoRecommendation")}</p> : <motion.div className="track-list" variants={listVariants} initial={reducedMotion ? false : "hidden"} animate="visible">{result.recommendations.map((track) => <SongCard key={track.track_id} track={track} active={audio.activeTrackId === track.track_id} elapsed={audio.elapsed} unavailable={audio.unavailableTrackId === track.track_id} onPreview={() => { void audio.toggle(track.track_id, track.preview_url) }} />)}</motion.div>}
