...
export function DiagnosticsPanel({ qualityNotes }: { qualityNotes: Record<string, unknown> }) {
  ...
  return <Collapsible className="mt-5 rounded-lg border p-3"><CollapsibleTrigger className="min-h-11 text-sm font-medium">{t("behindScenes")}</CollapsibleTrigger><CollapsibleContent className="pt-3"><dl className="grid gap-2 text-sm">{rows.map(([label, value]) => <div key={label} className="grid gap-1 sm:grid-cols-[10rem_1fr]"><dt className="font-medium">{label}</dt><dd className="break-words text-muted-foreground">{typeof value === "string" || typeof value === "number" ? String(value) : JSON.stringify(value)}</dd></div>)}</dl></CollapsibleContent></Collapsible>
}