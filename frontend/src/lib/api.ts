...
export async function jsonFetch<T>(url: string, options: JsonOptions = {}): Promise<T> {
  ...
  if (response.headers.get("content-type")?.includes("application/json")) {
    try { data = await response.json() } catch { /* retain fallback error */ }
  }
...
export function createPlaylist(payload: Record<string, unknown>) {
  return jsonFetch<Record<string, unknown>>("/create-playlist", { method: "POST", credentials: "include", body: JSON.stringify(payload) })
}
...
export function spotifyHealth() {
  return jsonFetch<SpotifyHealth>("/spotify/health")
}